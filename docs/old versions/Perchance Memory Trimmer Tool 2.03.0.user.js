...
(() => {
  ...
  var init_defaults = __esm({
    "src/defaults.js"() {
      DEFAULTS = {
        ...
        theme: "dark",
        // 'dark' | 'light' | 'custom'
        // Feature flags (patch: host-integration, safety-and-continuity)
        normalizeSeparators: true,
        // normalize CRLF on copy/export output
        showRepetitionBadge: true,
        // show risk badge after trim
        showAutomationBadge: true
        // show host connection badge in status bar
      };
    }
  });
  ...
  function loadCfg() {
    var _a;
    return { ...DEFAULTS, ...(_a = store.get(CFG_KEY)) != null ? _a : {} };
  }
  ...
  function getHostAnchors() {
    return {
      ...
      shortcutButtons: document.querySelector("#shortcutButtonsCtn"),
  ...
  function getAutomationState() {
    ...
    const hasMessageInput = anchors.messageInput instanceof HTMLTextAreaElement;
    const hasSendButton = anchors.sendButton instanceof HTMLButtonElement;
    const hasWindow = anchors.windows.length > 0;
    let status = "manual";
    if (hasMessageInput && hasSendButton) status = "connected";
    if (hasWindow) status = "window-detected";
    if (!hasMessageInput || !hasSendButton) status = "manual";
    return { status, hasMessageInput, hasSendButton, hasWindow, anchors };
  }
  ...
  function injectMiniToolbar(windowEl, options = {}) {
    ...
    if (header.querySelector('[data-pmt-toolbar="true"]')) {
      return header.querySelector('[data-pmt-toolbar="true"]');
    }
    ...
    (options.buttons || []).forEach(({ label, title, onClick }) => {
  ...
  function injectShortcutButton({ label = "\u{1F9E0} Trim mem", title = "Open the Memory Trimmer", onClick = null } = {}) {
    const container = document.querySelector("#shortcutButtonsCtn");
  ...
  __export(snapshot_exports, {
    ...
    toggleStarSnapshot: () => toggleStarSnapshot
  });
  ...
  var SNAPSHOT_KEY_PREFIX, MAX_UNSTARRED, SCHEMA_VERSION2;
  var init_snapshot = __esm({
    "src/core/snapshot.js"() {
      init_storage();
      init_helpers();
      SNAPSHOT_KEY_PREFIX = "pmt5_snaps_";
      MAX_UNSTARRED = 20;
      SCHEMA_VERSION2 = 1;
    }
  });
...
#pmt5-fab:hover  { background:#f3f4f6; border-color:#8c959f; color:#24292f; }
#pmt5-fab:active { background:#ebecf0; }
#pmt5-backdrop   { background: rgba(0,0,0,0.25); }
#pmt5-panel { background:#ffffff; border-color:#d0d7de; box-shadow:0 8px 40px rgba(0,0,0,0.18); }
#pmt5-topbar { background:#f6f8fa; border-bottom-color:#d0d7de; }
#pmt5-hdr    { background:#f6f8fa; }
...
#pmt5-body { scrollbar-color:#d0d7de transparent; }
.pmt5-sec  { border-bottom-color:#d0d7de; }
#pmt5-ta   { background:#f6f8fa; color:#24292f; border-color:#d0d7de; }
#pmt5-ta:focus { border-color:#0969da; }
#pmt5-ta::placeholder { color:#8c959f; }
.pmt5-pill { background:#f6f8fa; border-color:#d0d7de; color:#57606a; }
.pmt5-pill b { color:#24292f; }
.pmt5-og   { background:#f6f8fa; border-color:#d0d7de; }
...
.pmt5-sel, .pmt5-numbox { background:#ffffff; color:#24292f; border-color:#d0d7de; }
.pmt5-sel:focus, .pmt5-numbox:focus { border-color:#0969da; }
.pmt5-btn-undo { background:#f6f8fa; color:#24292f; border-color:#d0d7de; }
.pmt5-btn-prev { background:#f6f8fa; color:#24292f; border-color:#d0d7de; }
.pmt5-btn-clr  { color:#57606a; border-color:#d0d7de; }
.pmt5-btn-undo:hover:not(:disabled) { border-color:#8c959f; color:#24292f; }
.pmt5-btn-prev:hover:not(:disabled) { border-color:#8c959f; color:#24292f; }
#pmt5-rail { background:#f6f8fa; border-bottom-color:#d0d7de; }
...
.pmt5-c-hl   { background:rgba(9,105,218,0.12); color:#0969da; }
#pmt5-settings { scrollbar-color:#d0d7de transparent; }
.pmt5-stg-row  { background:#f6f8fa; border-color:#d0d7de; }
...
`;
  ...
  function estTokens(text) {
    if (typeof window !== "undefined" && typeof window.countTokens === "function") {
      try {
        return window.countTokens(text);
      } catch {
      }
    }
  ...
  function renderResultTab({
    ...
    const parts = entries.map((entry) => {
      ...
      return `<span class="${ns}-c-kept ${ns}-entry" ${visible ? "" : 'style="display:none"'} ${isPinned2 ? `class="${ns}-c-kept ${ns}-entry ${ns}-entry-pinned"` : ""}>${controls}${chips ? `<span class="${ns}-chips">${chips}</span>` : ""}${highlight(entry, q2, ns)}</span>`;
    });
  ...
  function renderRemovedTab({ outEl, searchCt, entries, filterQ: filterQ2, ns = "pmt5" }) {
    const q2 = filterQ2.toLowerCase().trim();
    if (!entries.length) {
    ...
    const parts = entries.map((entry) => {
      const visible = !q2 || entry.toLowerCase().includes(q2);
      if (visible) matched++;
      return `<span class="${ns}-c-gone ${ns}-entry" ${visible ? "" : 'style="display:none"'}>${highlight(entry, q2, ns)}</span>`;
    });
    outEl.innerHTML = parts.join(`<span class="${ns}-c-sep">
...
${lines.join("\n")}`;
  ...
  async function applyMemoryWindowText(windowEl, outgoingText, scopeId, opts = {}) {
    ...
    return { status: ok ? "verified" : "mismatch", snapshot };
  }
  ...
  function buildPrompt(question, memoryText, scope) {
    return `You are a helpful memory-inspection assistant for a Perchance AI chat tool called PMT.

The user has asked a question about what is currently stored in their Perchance memory (the /mem block).
Answer ONLY from the memory text below. Do not speculate beyond what is written.
...
Scope: ${scope.scopeLabel} (confidence: ${scope.confidence})
...
User question: ${question}`;
  ...
  function openQaPopup({ entries, selected = [], scope }) {
    ...
    popup.innerHTML = `
    <div id="${QA_ID}-header">
      <span id="${QA_ID}-title">\u{1F9E0} Memory Q&amp;A</span>
      <span id="${QA_ID}-scope" title="Scope: ${escHtml(scope.scopeLabel)}">${escHtml(scope.confidence)} scope</span>
      <button id="${QA_ID}-close" title="Close">\u2715</button>
    </div>
    <div id="${QA_ID}-scope-sel">
      <label style="font-size:11px;color:#8b949e">Ask about:
        <select id="${QA_ID}-scope-mode" style="margin-left:6px;background:#0d1117;color:#c9d1d9;border:1px solid #30363d;border-radius:4px;padding:2px 6px;font-size:11px">
    ...
    <div id="${QA_ID}-body">
      <div id="${QA_ID}-answer" class="${NS}-hint">Ask a question about what is stored in memory\u2026</div>
      <div id="${QA_ID}-citations"></div>
    </div>
    <div id="${QA_ID}-footer">
      <input id="${QA_ID}-input" type="text" placeholder="e.g. What does memory say about Alice?" autocomplete="off" spellcheck="false">
      <button id="${QA_ID}-ask">Ask</button>
    </div>`;
    popup.style.cssText = `
    position:fixed; bottom:96px; right:18px; width:min(420px,94vw);
    background:#0d1117; border:1px solid #30363d; border-radius:10px;
    box-shadow:0 8px 40px rgba(0,0,0,0.65); z-index:2147483645;
    display:flex; flex-direction:column; font-family:system-ui,sans-serif;
    font-size:13px; overflow:hidden; max-height:70vh;`;
    const header = popup.querySelector(`#${QA_ID}-header`);
    header.style.cssText = "display:flex;align-items:center;gap:8px;padding:10px 12px;background:#161b22;border-bottom:1px solid #21262d;flex-shrink:0";
    popup.querySelector(`#${QA_ID}-title`).style.cssText = "font-weight:700;color:#e6edf3;flex:1";
    popup.querySelector(`#${QA_ID}-scope`).style.cssText = "font-size:10px;color:#6e7681;background:#21262d;border-radius:10px;padding:2px 7px";
    const closeBtn = popup.querySelector(`#${QA_ID}-close`);
    closeBtn.style.cssText = "all:unset;box-sizing:border-box;cursor:pointer;color:#6e7681;font-size:16px;padding:2px 6px;border-radius:4px";
    closeBtn.addEventListener("click", () => popup.remove());
    popup.querySelector(`#${QA_ID}-scope-sel`).style.cssText = "padding:7px 12px;border-bottom:1px solid #21262d;background:#161b22;flex-shrink:0";
    const answerEl = popup.querySelector(`#${QA_ID}-answer`);
    answerEl.style.cssText = "padding:10px 12px;color:#c9d1d9;line-height:1.65;overflow-y:auto;flex:1;white-space:pre-wrap;word-break:break-word";
    const citEl = popup.querySelector(`#${QA_ID}-citations`);
    citEl.style.cssText = "padding:0 12px 8px;font-size:11px;color:#6e7681";
    const bodyEl = popup.querySelector(`#${QA_ID}-body`);
    bodyEl.style.cssText = "display:flex;flex-direction:column;flex:1;overflow:hidden;min-height:80px";
    const footer = popup.querySelector(`#${QA_ID}-footer`);
    footer.style.cssText = "display:flex;gap:7px;padding:8px 12px;border-top:1px solid #21262d;background:#161b22;flex-shrink:0";
    const inputEl = popup.querySelector(`#${QA_ID}-input`);
    inputEl.style.cssText = "flex:1;background:#010409;color:#c9d1d9;border:1px solid #30363d;border-radius:6px;padding:6px 9px;font-size:12px;outline:none";
    inputEl.addEventListener("focus", () => inputEl.style.borderColor = "#388bfd");
    inputEl.addEventListener("blur", () => inputEl.style.borderColor = "#30363d");
    const askBtn = popup.querySelector(`#${QA_ID}-ask`);
    askBtn.style.cssText = "background:#238636;color:#fff;border:none;border-radius:6px;padding:6px 14px;font-weight:600;font-size:12px;cursor:pointer";
    ...
    async function doAsk() {
      ...
      if (!question) return;
      const scopeMode = (_b = (_a = popup.querySelector(`#${QA_ID}-scope-mode`)) == null ? void 0 : _a.value) != null ? _b : "all";
      const useEntries = scopeMode === "sel" && selected.length ? selected : entries;
      const memText = serializeEntries(useEntries);
      askBtn.disabled = true;
      askBtn.textContent = "\u2026";
      answerEl.textContent = "Thinking\u2026";
      answerEl.style.color = "#6e7681";
      citEl.textContent = "";
      try {
        const response = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 1e3,
            messages: [{ role: "user", content: buildPrompt(question, memText, scope) }]
          })
        });
        if (!response.ok) {
          const err = await response.json().catch(() => ({}));
          throw new Error((_d = (_c = err == null ? void 0 : err.error) == null ? void 0 : _c.message) != null ? _d : `HTTP ${response.status}`);
        }
        const data = await response.json();
        const answer = (_g = (_f = (_e = data.content) == null ? void 0 : _e.find((b) => b.type === "text")) == null ? void 0 : _f.text) != null ? _g : "(No response)";
        answerEl.textContent = answer;
        answerEl.style.color = "#c9d1d9";
        citEl.textContent = `${useEntries.length} entries searched \xB7 ${memText.length} chars`;
      } catch (err) {
        answerEl.textContent = `Error: ${err.message}`;
        answerEl.style.color = "#f85149";
      } finally {
  ...
  function openBubbleMap({ entries, continuityMap: continuityMap2 = {}, pinnedIds = /* @__PURE__ */ new Set(), getEntryId: getEntryId2, minClusterSize = 1 }) {
    const existing = document.getElementById(MAP_ID);
    if (existing) {
      existing.remove();
      return;
    }
    const bubbles = entries.filter((_, i) => i % Math.max(1, minClusterSize) === 0).map((entry, i) => {
      var _a;
      const entryId = getEntryId2(entry);
      const contInfo = (_a = continuityMap2[entryId]) != null ? _a : { label: "low", score: 0 };
      const tokens = getNativeTokenCount(entry);
      const radius = Math.max(18, Math.min(55, Math.sqrt(tokens) * 3.5));
      return {
        entry,
        entryId,
        radius,
        ...
        isPinned: pinnedIds.has(entryId),
        color: bubbleColor(contInfo.label, pinnedIds.has(entryId)),
    ...
    for (let tick = 0; tick < 30; tick++) {
      for (let i = 0; i < bubbles.length; i++) {
        ...
        bubbles[i].x = Math.max(bubbles[i].radius, Math.min(820 - bubbles[i].radius, bubbles[i].x));
        bubbles[i].y = Math.max(bubbles[i].radius, Math.min(500 - bubbles[i].radius, bubbles[i].y));
      }
    }
    ...
    font-family:system-ui,sans-serif; animation:pmt5-fade-in 0.2s ease;`;
    overlay.innerHTML = `
    <div style="position:relative;background:#0d1117;border:1px solid #30363d;border-radius:10px;overflow:hidden;width:min(860px,97vw);max-height:92vh;display:flex;flex-direction:column">
      <div style="display:flex;align-items:center;gap:10px;padding:9px 14px;background:#161b22;border-bottom:1px solid #21262d;flex-shrink:0">
        <span style="font-weight:700;color:#e6edf3;flex:1">\u{1FAE7} Bubble Map \u2014 ${bubbles.length} entries</span>
        <label style="font-size:11px;color:#8b949e">Min cluster size:
          <input id="${MAP_ID}-slider" type="range" min="1" max="5" value="${minClusterSize}"
            style="width:80px;accent-color:#388bfd;vertical-align:middle;margin-left:4px">
          <span id="${MAP_ID}-slider-val">${minClusterSize}</span>
        </label>
        <span style="font-size:11px;color:#6e7681">Drag bubbles \xB7 size=tokens \xB7 colour=continuity</span>
        <button id="${MAP_ID}-close" style="all:unset;box-sizing:border-box;cursor:pointer;color:#6e7681;font-size:18px;padding:2px 7px;border-radius:5px" title="Close map">\u2715</button>
      </div>
      <div style="flex:1;overflow:auto;position:relative">
        <canvas id="${MAP_ID}-canvas" width="860" height="520" style="display:block;cursor:grab;touch-action:none"></canvas>
      </div>
      <div id="${MAP_ID}-tooltip" style="position:fixed;background:#161b22;border:1px solid #30363d;border-radius:7px;padding:8px 12px;font-size:12px;color:#c9d1d9;max-width:280px;pointer-events:none;display:none;z-index:2147483647;line-height:1.5"></div>
    </div>`;
    ...
    function hitTest(px, py) {
      return bubbles.find((b) => Math.hypot(px - b.x, py - b.y) <= b.radius);
    }
    ...
    canvas.addEventListener("pointermove", (e) => {
      const { x, y } = pointerToCanvas(e);
      if (dragging) {
        dragging.x = Math.max(dragging.radius, Math.min(canvas.width - dragging.radius, x - dragOX));
        dragging.y = Math.max(dragging.radius, Math.min(canvas.height - dragging.radius, y - dragOY));
        draw();
      } else {
        const hit = hitTest(x, y);
        if (hit) {
          ...
          tooltip.innerHTML = `<b>${escHtml(hit.entry.slice(0, 100))}</b><br>Tokens: ${hit.tokens} \xB7 ${hit.label} continuity${hit.isPinned ? " \xB7 \u{1F4CC} pinned" : ""}`;
        } else {
    ...
    slider.addEventListener("input", () => {
      ...
      overlay.remove();
      openBubbleMap({ entries, continuityMap: continuityMap2, pinnedIds, getEntryId: getEntryId2, minClusterSize: parseInt(slider.value) });
    });
  ...
  function setTokenSource(src) {
    _source = ["auto", "native", "heuristic", "exact"].includes(src) ? src : "auto";
  }
...
<ol style="margin:8px 0 0 16px;padding:0;line-height:1.8">
  <li>In Perchance, type <code>/mem</code> and send \u2014 or click <b>\u2B07 Fetch</b> to load memory automatically.</li>
  <li>Click <b>Trim</b> to remove old or duplicate entries.</li>
  <li>Review the <b>Result</b> tab. Pin \u{1F4CC} important entries to protect them.</li>
  <li>Click <b>\u2B06 Apply</b> to write trimmed memory back, or <b>Copy</b> and paste manually.</li>
  <li>Use <b>\u{1F4F7} Snap</b> before any risky change \u2014 it saves a restore point.</li>
</ol>

<b>New features:</b> Health score, near-duplicates, repetition risk badge, continuity scoring, lore draft, and more \u2014 all in the tabs above.
`;
  function showOnboardingBanner(panel, onDismiss) {
    ...
    const topbar = panel.querySelector(`#${NS}-topbar`);
  ...
  function openPanel() {
    ...
    <div id="${NS}-header">
      ...
      <span id="${NS}-kbd">Ctrl+Enter=Trim \xB7 Esc=Close \xB7 Ctrl+/=Diag</span>
    ...
    <div id="${NS}-actions-secondary" role="toolbar" aria-label="More actions">
      <button class="pmt5-btn-tool accent" id="${NS}-fetch"       title="Fetch /mem from Perchance">\u2B07 Fetch</button>
      <button class="pmt5-btn-tool"        id="${NS}-prev"   disabled title="Preview diff">Preview</button>
      <button class="pmt5-btn-tool"        id="${NS}-snap"   disabled title="Save snapshot">\u{1F4F7} Snap</button>
      <button class="pmt5-btn-tool"        id="${NS}-exp-kept"    disabled title="Download kept entries">\u2B07 Kept</button>
      <button class="pmt5-btn-tool"        id="${NS}-exp-removed" disabled title="Download removed entries">\u2B07 Removed</button>
      <button class="pmt5-btn-tool lore"   id="${NS}-lore"  disabled title="Copy as lore draft">\u{1F4D6} Lore</button>
      <button class="pmt5-btn-tool"        id="${NS}-sort-sim"    disabled title="Group similar entries">\u{1F500} Group</button>
      <button class="pmt5-btn-tool qa"     id="${NS}-qa"          title="Ask questions about memory">\u{1F4AC} Q&amp;A</button>
      <button class="pmt5-btn-tool"        id="${NS}-map"   disabled title="Bubble map visual view">\u{1FAE7} Map</button>
      <button class="pmt5-btn-tool danger" id="${NS}-clr"         title="Clear all">Clear</button>
    </div>
    ...
    <div id="${NS}-tabs" role="tablist">
      <div class="pmt5-tab pmt5-active" data-tab="result"   role="tab" aria-selected="true">Result</div>
    ...
    <div id="${NS}-status" role="status">
      ...
      <span id="${NS}-undo-label" style="display:none"></span>
    </div>
  `;
    ...
    function updateDiagDrawer() {
      const { getAutomationState: gas } = { getAutomationState: () => ({ status: "unknown" }) };
      try {
        Promise.resolve().then(() => (init_helpers(), helpers_exports)).then(({ getAutomationState: getAutomationState2 }) => {
          diagHost.textContent = getAutomationState2().status;
        }).catch(() => {
        });
      } catch {
      }
    ...
    if (runSmokeBtn) {
      runSmokeBtn.addEventListener("click", async () => {
        ...
        diagSmoke.style.color = r.failCount > 0 ? "#f85149" : "#3fb950";
    ...
    if (!currentEntries.length && activeTab !== "result" && activeTab !== "settings") {
      activeTab = "result";
    }
    ...
    function _refreshUndo() {
      ...
      if (undoLabel) {
        ...
        undoLabel.style.display = n > 0 ? "inline" : "none";
      }
    }
    ...
    function renderOutput() {
      ...
      if (activeTab === "result") {
        ...
        renderHealthTab({ outEl, healthResult, ns: NS });
      } else if (activeTab === "dups") {
        renderDupsTab({ outEl, clusters: nearDupClusters, ns: NS });
    ...
    function _renderAnalyse() {
      ...
      if (healthResult) {
        ...
        lines.push(`<span class="${NS}-c-${hc}"><b>\u{1F3E5} Health: ${healthResult.label.toUpperCase()}</b> (${healthResult.score}/100 \xB7 ${healthResult.totalTokens}t)</span>`);
        healthResult.reasons.forEach((r) => lines.push(`<span class="${NS}-c-${hc}">  \xB7 ${r}</span>`));
        if (healthResult.suggestions.length) {
          lines.push("");
          healthResult.suggestions.forEach((s) => lines.push(`<span class="${NS}-c-ok">  \u2192 ${s}</span>`));
        }
      }
      if (repPill.style.display !== "none") {
        lines.push("", `<span class="${NS}-c-sep">${"\u2500".repeat(40)}</span>`);
        lines.push(`<span class="${NS}-c-warn"><b>\u{1F525} Repetition risk: ${(_a2 = repVal == null ? void 0 : repVal.textContent) != null ? _a2 : "\u2014"}</b></span>`);
      }
      if (nearDupClusters.length) {
        lines.push("", `<span class="${NS}-c-sep">${"\u2500".repeat(40)}</span>`);
        lines.push(`<span class="${NS}-c-warn"><b>\u{1F46F} Near-duplicates: ${nearDupClusters.length} cluster${nearDupClusters.length !== 1 ? "s" : ""}</b></span>`);
        nearDupClusters.slice(0, 5).forEach((cl, i) => {
          lines.push(`<span class="${NS}-c-kept">  Cluster ${i + 1} (${Math.round(cl.maxSimilarity * 100)}%): ${cl.entries[0].slice(0, 80)}\u2026</span>`);
        });
      }
      if (conflictList.length) {
        lines.push("", `<span class="${NS}-c-sep">${"\u2500".repeat(40)}</span>`);
        lines.push(`<span class="${NS}-c-warn"><b>\u26A0 Conflicts: ${conflictList.length}</b></span>`);
        conflictList.slice(0, 5).forEach((c) => {
          lines.push(`<span class="${NS}-c-warn">  ${c.severity}: ${c.reasons[0]}</span>`);
        });
      }
      if (!lines.length) lines.push(`<span class="${NS}-hint">Run Trim first to analyse memory.</span>`);
      outEl.innerHTML = lines.join("\n");
    }
    function _renderCurate() {
      const lines = [];
      if (topicGroups) {
        lines.push(`<span class="${NS}-c-ok"><b>\u{1F4DA} Topic Groups</b></span>`);
        const render = (label, groups) => {
          if (!groups.length) return;
          lines.push(`<span class="${NS}-c-sep">${label}</span>`);
          groups.slice(0, 5).forEach(
            ([name, items]) => lines.push(`<span class="${NS}-c-kept">  ${name} \u2014 ${items.length} entries</span>`)
          );
        };
      ...
      if (storyBeats.length) {
        lines.push("", `<span class="${NS}-c-sep">${"\u2500".repeat(40)}</span>`);
        lines.push(`<span class="${NS}-c-ok"><b>\u{1F4C5} Story Beats (${storyBeats.length})</b></span>`);
        const icons = { event: "\u26A1", relationship_change: "\u{1F49E}", world_fact: "\u{1F30D}", hook: "\u2753", misc: "\xB7" };
        storyBeats.slice(0, 8).forEach(
          (b) => lines.push(`<span class="${NS}-c-kept">${icons[b.beatType] || "\xB7"} ${b.entry.slice(0, 120)}</span>`)
        );
      }
      ...
      if (ctx && currentEntries.length) {
        ...
        if (scored.length) {
          lines.push("", `<span class="${NS}-c-sep">${"\u2500".repeat(40)}</span>`);
          lines.push(`<span class="${NS}-c-ok"><b>\u{1F3AF} Likely Relevant (from recent chat)</b></span>`);
          scored.forEach((s) => lines.push(`<span class="${NS}-c-kept">  ${s.entry.slice(0, 120)}</span>`));
        }
      }
      if (currentEntries.length) {
        lines.push("", `<span class="${NS}-c-sep">${"\u2500".repeat(40)}</span>`);
        lines.push(`<span class="${NS}-c-ok"><b>\u{1F504} Compare vs Lore / Summary</b></span>`);
        lines.push(`<span class="${NS}-hint">  Expand "More \u25BE" \u2192 use \u{1FAE7} Map or paste in Compare tab</span>`);
        lines.push(`<span class="${NS}-hint">  (Open the Compare panel via the secondary actions)</span>`);
      }
      if (!lines.length) lines.push(`<span class="${NS}-hint">Run Trim first to see curation insights.</span>`);
      outEl.innerHTML = lines.join("\n");
    }
    function _renderConflicts() {
      const { escHtml: esc } = { escHtml: (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") };
      if (!conflictList.length) {
        outEl.innerHTML = `<span class="${NS}-hint">No conflicts detected in current kept entries.</span>`;
        return;
      }
      outEl.innerHTML = conflictList.slice(0, PERF_CAPS.conflictLimit).map(
        (c) => `<span class="${NS}-c-warn">\u26A0 ${esc(c.severity.toUpperCase())} \u2014 ${esc(c.reasons[0])}</span>
...
`);
    ...
    function _renderTopics() {
      if (!topicGroups) {
        outEl.innerHTML = `<span class="${NS}-hint">Run Trim first to generate topic groups.</span>`;
        return;
      }
      const lines = [];
      const render = (label, groups) => {
        if (!groups.length) return;
        lines.push(`<span class="${NS}-c-ok"><b>${label}</b></span>`);
        groups.slice(0, 10).forEach(
          ([name, items]) => lines.push(`<span class="${NS}-c-kept">  ${name} (${items.length} entries)</span>`)
        );
        lines.push("");
      };
    ...
    function _renderTimeline() {
      if (!storyBeats.length) {
        outEl.innerHTML = `<span class="${NS}-hint">Run Trim first to extract story beats.</span>`;
        return;
      }
      const beats = storyBeats.slice(0, PERF_CAPS.beatLimit);
      const typeIcon = { event: "\u26A1", relationship_change: "\u{1F49E}", world_fact: "\u{1F30D}", hook: "\u2753", misc: "\xB7" };
      outEl.innerHTML = beats.map(
        (b) => {
          var _a2;
          return `<span class="${NS}-c-kept">${(_a2 = typeIcon[b.beatType]) != null ? _a2 : "\xB7"} <b>[${b.beatType.replace("_", " ")}]</b> ${b.entry.slice(0, 140)}</span>`;
        }
      ).join(`
<span class="${NS}-c-sep">\u2500</span>
`);
    }
    function _renderRelevance() {
      const ctx = readRecentContext();
      if (!ctx || !currentEntries.length) {
        outEl.innerHTML = `<span class="${NS}-hint">${!currentEntries.length ? "Run Trim first." : "No recent Perchance chat context found \u2014 open PMT within an active chat."}</span>`;
        return;
      }
      const scored = scoreRelevance(currentEntries, ctx, getEntryId).slice(0, 30);
      outEl.innerHTML = scored.map(
        (s) => `<span class="${NS}-c-kept"><b>${String(Math.round(s.score * 100) / 100).padStart(4)}</b> ${s.entry.slice(0, 150)}</span>`
      ).join("\n");
    }
    ...
    tabsEl.addEventListener("click", (e) => {
      ...
      if (tab == null ? void 0 : tab.dataset.tab) switchTab2(tab.dataset.tab);
    });
    ...
    function buildSettings() {
      ...
      settingEl.innerHTML = [
        ...
        mkGroupRow(NS, "Display", [
          ...
          <div>
            ...
            <div class="pmt5-stg-sub">Auto prefers native (window.countTokens), then heuristic (~4 chars/token)</div>
          </div>
        ...
        // ── Group: Automation ─────────────────────────────────────────────
        mkGroupRow(NS, "Automation", [
          mkToggleRow(
            NS,
            "stg-auto",
            "Show host connection badge",
            "Displays Perchance automation status in the header",
            (_e = cfg.showAutomationBadge) != null ? _e : true
          )
        ]),
        // ── Group: Continuity ─────────────────────────────────────────────
        mkGroupRow(NS, "Continuity", []),
        // ── Group: Diagnostics ────────────────────────────────────────────
        mkGroupRow(NS, "Diagnostics", [
          mkToggleRow(
            ...
            "stg-debug",
            "Debug mode (Ctrl+/)",
            "Opens the diagnostics drawer \u2014 host status, storage, smoke tests",
            (_f = cfg.debugMode) != null ? _f : false
          )
        ]),
        // ── Group: Theme ──────────────────────────────────────────────────
        mkGroupRow(NS, "Theme", [
          mkThemeRow(NS, (_g = cfg.theme) != null ? _g : "dark", escHtml(customCss))
        ]),
        // ── Group: Window ─────────────────────────────────────────────────
        mkGroupRow(NS, "Window", [
          mkToggleRow(
            NS,
            "stg-pos",
      ...
      bindToggle(
        ...
        "showRepetitionBadge",
        (on) => {
          if (!on) repPill.style.display = "none";
        }
      );
      bindToggle("stg-auto", "showAutomationBadge");
    ...
    btnRun.addEventListener("click", () => {
      ...
      switchTab2(cfg.previewOnTrim ? "preview" : "result");
    });
    ...
    btnApplyMem.addEventListener("click", async () => {
      ...
      const { status } = await applyMemoryWindowText(windowEl, outText, scope.scopeId);
      btnApplyMem.disabled = false;
      ...
      if (status === "mismatch") {
        showRecovery("Apply mismatch detected \u2014 restore last known good?", "Restore", () => {
          const { getLastSnapshot: getLastSnapshot2 } = (init_snapshot(), __toCommonJS(snapshot_exports));
          const snap = getLastSnapshot2 == null ? void 0 : getLastSnapshot2(scope.scopeId);
          if (snap) {
    ...
    btnSortSim.addEventListener("click", () => {
      ...
      switchTab2("result");
      setStatus(`Grouped ${groups.length} similarity clusters.`, "ok", 4e3);
    });
    ...
    if (runCompareBtn) {
      runCompareBtn.addEventListener("click", async () => {
        ...
        const lines = [
          ...
          ...result.loreSuggestions.slice(0, 5).map((e) => `<span class="${NS}-c-kept">  ${e.slice(0, 120)}</span>`)
        ];
        outEl.style.display = "";
    ...
    function _handleAnnotationClick(entryId) {
      ...
      const note = prompt("Note for this entry (leave blank to clear):", (_a2 = existing == null ? void 0 : existing.note) != null ? _a2 : "");
      if (note === null) return;
      saveAnnotation(scope.scopeId, entryId, { note, flags: (_b2 = existing == null ? void 0 : existing.flags) != null ? _b2 : [] });
      setStatus(note ? "Note saved." : "Note cleared.", "ok", 2e3);
      renderOutput();
    }
    ...
    btnPrev.addEventListener("click", () => switchTab2("preview"));
  ...
  // src/app/mount.js
  init_constants();
  function createMountObserver({ sentinel, fab, styleEl }) {
...
/* \u2500\u2500 CSS token layer (custom-theme-authoring patch) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500 */
#pmt5-panel, #pmt5-fab {
