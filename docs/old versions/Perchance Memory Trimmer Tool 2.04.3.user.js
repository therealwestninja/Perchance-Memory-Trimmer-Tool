...
(() => {
  ...
  function getNativeTokenCount(text) {
    try {
      if (getTokenSource() !== "heuristic" && typeof window.countTokens === "function") {
        const value = window.countTokens(String(text != null ? text : ""));
        if (Number.isFinite(value) && value >= 0) return value;
      }
    } catch {
    }
    if (getTokenSource() === "native") return 0;
  ...
  function runSmokeTests() {
    ...
    try {
      ...
      const fakeHeader = document.createElement("div");
      fakeHeader.id = `${NS}-header`;
      fakePanel.appendChild(fakeHeader);
      let dismissed = false;
      const banner = showOnboardingBanner(fakePanel, () => { dismissed = true; }, { automation: false, memoryWindowAccess: false, qaRemote: false, miniToolbar: false });
      const helpBody = banner == null ? void 0 : banner.querySelector('[data-pmt-help="body"]');
      const closeBtn = banner == null ? void 0 : banner.querySelector(`#${NS}-onboard-close`);
      if (closeBtn) closeBtn.click();
      const ok = !!banner && !!helpBody && helpBody.innerHTML.includes("paste <code>/mem</code> manually") && !!closeBtn && dismissed;
      record(results, "onboarding-banner-exposes-capability-aware-help", ok, ok ? "banner + help body + dismiss path ok" : `banner=${!!banner} help=${!!helpBody} close=${!!closeBtn} dismissed=${dismissed}`);
      if (banner && banner.isConnected) banner.remove();
    } catch (e) {
      record(results, "onboarding-banner-exposes-capability-aware-help", false, String(e));
    }
    return {
      version: VERSION,
...
`;
  ...
  function buildChips(opts, ns) {
    const chips = [];
    const { isPinned: isPinned2, isProtected, isHotspot, showTokens, continuityLabel, loreLabel, entryText, hasAnnotation, entryId = "" } = opts;
    ...
    if (hasAnnotation) chips.push(`<button type="button" class="${ns}-annot-badge ${ns}-btn-micro" data-entry-id="${escHtml(entryId)}" title="Edit annotation">\u{1F4DD}</button>`);
  ...
  function renderResultTab({
    ...
    let matched = 0;
    const parts = entries.map((entry, entryIdx) => {
      var _a, _b, _c;
      ...
      const annotBtn = entryId ? `<button class="${ns}-btn-micro ${ns}-annot-open" data-entry-id="${escHtml(entryId)}" title="Add or edit annotation">\u{1F4DD}</button>` : "";
      const controls = selChk || pinBtn || annotBtn ? `<span class="${ns}-entry-controls">${selChk}${pinBtn}${annotBtn}</span>` : "";
...
` + topMatches.map((r) => `\u2022 ${r.entry.slice(0, 120)}${r.entry.length > 120 ? "\u2026" : ""}`).join("\n");
  ...
  function openQaPopup({ entries, selected = [], scope }) {
    ...
    popup.className = "pmt5-qa-popup-shell";
    popup.innerHTML = `
    ...
    <div id="${QA_ID}-privacy-notice" class="pmt5-qa-notice"${hasKey ? ' hidden' : ""}>
    ...
    <div id="${QA_ID}-key-row" class="pmt5-qa-key-row">
      ...
      <button id="${QA_ID}-key-clear" class="pmt5-qa-btn"${hasKey ? "" : " hidden"}>Clear</button>
    </div>
  ...
  function openBubbleMap({ entries, continuityMap: continuityMap2 = {}, pinnedIds = /* @__PURE__ */ new Set(), getEntryId: getEntryId2, minClusterSize = 1, onApplyOrder = null }) {
    ...
    overlay.classList.add("pmt5-bubble-overlay-shell");
    overlay.innerHTML = `
    <div class="pmt5-bubble-shell pmt5-bubble-shell-sized">
      <div class="pmt5-bubble-hdr">
        ...
        <label class="pmt5-bubble-cluster-label">Min cluster:
          <input id="${MAP_ID}-slider" class="pmt5-bubble-slider" type="range" min="1" max="5" value="${minClusterSize}">
          <span id="${MAP_ID}-slider-val">${minClusterSize}</span>
        </label>
        <button id="${MAP_ID}-color-toggle" class="pmt5-bubble-close pmt5-bubble-btn-compact" title="Toggle colour mode between continuity priority and story beat type">\u{1F3A8} Continuity</button>
        ...
        <button id="${MAP_ID}-reset" class="pmt5-bubble-close pmt5-bubble-reset" title="Reset layout">\u21BA</button>
        <button id="${MAP_ID}-close" class="pmt5-bubble-close" title="Close map">\u2715</button>
      </div>
      <div class="pmt5-bubble-meta">
        ...
        <span id="${MAP_ID}-order-hint" class="pmt5-bubble-order-hint" hidden>\u2195 Order changed \u2014 click Apply Order to commit to workspace</span>
      </div>
      <div class="pmt5-bubble-canvas-wrap">
        <canvas id="${MAP_ID}-canvas" width="${CANVAS_W}" height="${CANVAS_H}"
          ...
          class="pmt5-bubble-canvas">
          Interactive memory bubble timeline.
        </canvas>
      </div>
      <div id="${MAP_ID}-legend" class="pmt5-bubble-legend"></div>
      <div id="${MAP_ID}-tooltip" class="pmt5-bubble-tooltip" hidden></div>
    </div>`;
  ...
  function getTokenSource() {
    const valid = ["auto", "native", "heuristic"];
    return valid.includes(_source) ? _source : "auto";
  }
  ...
  function getTokenSourceLabel() {
    ...
    if (typeof window !== "undefined" && typeof window.countTokens === "function") {
      return source === "heuristic" ? "estimate" : "native";
    }
    return "estimate";
  }
  ...
  function buildHelpContent(caps = null) {
    const automationLine = caps && !caps.automation ? `<b>Fetch</b> is unavailable on this page right now — paste <code>/mem</code> manually instead.` : `Use <b>⬇ Fetch</b> when Perchance controls are detected.`;
    const applyLine = caps && !caps.memoryWindowAccess ? `<b>⬆ Apply</b> may be unavailable until a memory window is open — <b>Copy</b> still works.` : `Click <b>⬆ Apply</b> to write back automatically, or <b>Copy</b> to paste manually.`;
    const qaLine = caps && !caps.qaRemote ? `<b>Q&amp;A assistant</b>: local mode works now; remote answers unlock after you add an Anthropic API key in Settings.` : `<b>Q&amp;A assistant</b>: requires an Anthropic API key in Settings for remote answers.`;
    const toolbarLine = caps && !caps.miniToolbar ? `<b>Mini toolbar</b>: only appears when a Perchance memory window is visible.` : `<b>Mini toolbar</b>: can be injected into the Perchance memory window from Settings.`;
    return `
<b>Welcome to Memory Trimmer!</b>

Here's how it works:
<ol style="margin:8px 0 0 16px;padding:0;line-height:1.8">
  <li>Paste your <code>/mem</code> text into the workspace — ${automationLine}</li>
  <li>Click <b>Trim</b> to remove old, duplicate, or oversized entries.</li>
  <li>The <b>🧠 Timeline</b> tab shows your entries clustered by similarity — drag to reorder.</li>
  <li>Pin 📌 important entries to protect them from future trims.</li>
  <li>${applyLine}</li>
  <li>Save a <b>📷 Snapshot</b> before any risky change — restore it any time from the Snapshots tab.</li>
</ol>
...
`;
  ...
  function showOnboardingBanner(panel, onDismiss, caps = null) {
    if (!panel) return null;
    const existingInPanel = panel.querySelector ? panel.querySelector(`#${NS}-onboard`) : null;
    if (existingInPanel) return existingInPanel;
    const existing = document.getElementById(`${NS}-onboard`);
    if (existing) return existing;
    const banner = document.createElement("div");
    banner.id = `${NS}-onboard`;
    banner.innerHTML = `
    <div class="pmt5-onboard-inner">
      <div class="pmt5-onboard-body" data-pmt-help="body">${buildHelpContent(caps)}</div>
      <button id="${NS}-onboard-close" title="Dismiss" type="button" aria-label="Dismiss onboarding">✕</button>
    </div>`;
    const topbar = panel.querySelector ? panel.querySelector(`#${NS}-header`) : null;
    if ((topbar == null ? void 0 : topbar.parentNode) === panel && (topbar == null ? void 0 : topbar.nextSibling)) panel.insertBefore(banner, topbar.nextSibling);
    else panel.appendChild(banner);
    const closeBtn = banner.querySelector(`#${NS}-onboard-close`) || banner.querySelector("button");
    if (closeBtn) {
  ...
  function openPanel() {
    ...
    <div id="${NS}-output-zone" role="tabpanel">
      ...
      <div id="${NS}-diag-drawer" aria-label="Diagnostics">
        ...
        <div class="pmt5-diag-row" style="flex-direction:column;gap:2px"><span class="pmt5-diag-key">Smoke failures:</span><span class="pmt5-diag-val" id="${NS}-diag-smoke-failures" style="font-size:10px;white-space:pre-wrap;opacity:0.9">\u2014</span></div>
  ...
  `;
    ...
    function applyCapabilityGating() {
      const liveCaps = buildCapabilities(cfg);
      if (btnFetch) {
        btnFetch.disabled = !liveCaps.automation;
        btnFetch.title = liveCaps.automation ? "Fetch /mem from Perchance" : "Fetch unavailable — Perchance host controls not detected. Use manual paste.";
      }
      if (btnApplyMem) {
        const hasWorkspace = currentEntries.length > 0;
        btnApplyMem.disabled = !liveCaps.memoryWindowAccess || !hasWorkspace || _applyBusy;
        btnApplyMem.title = !liveCaps.memoryWindowAccess ? "Apply unavailable — no Perchance memory window detected. Use Copy instead." : "Apply to Perchance (Ctrl+Shift+Enter)";
      }
      if (btnQa && !liveCaps.qaRemote) {
        btnQa.title = "Q&A — Local mode only (no API key). Add a key in Settings › Experimental › Q&A Assistant for remote mode.";
      }
      if (btnMap) btnMap.disabled = !currentEntries.length;
      if (btnSortSim) btnSortSim.disabled = !currentEntries.length;
      if (btnLore) btnLore.disabled = !currentEntries.length;
      if (btnSnap) btnSnap.disabled = !currentEntries.length;
      if (btnExpKept) btnExpKept.disabled = !currentEntries.length;
      return liveCaps;
    }
    ...
    const onboardingCaps = buildCapabilities(cfg);
    let onboardingBanner = null;
    if (isFirstRun()) {
      onboardingBanner = showOnboardingBanner(panel, () => {
    ...
    const diagSmokeFailures = q("diag-smoke-failures");
    let lastSmokeResults = null;
    function formatSmokeSummary(smoke) {
      if (!smoke) return "not run";
      const total = (smoke.passCount || 0) + (smoke.failCount || 0);
      return `${smoke.passCount}/${total} passed`;
    }
    function summarizeSmokeFailures(smoke, max = 3) {
      if (!smoke || !Array.isArray(smoke.results)) return "\u2014";
      const failed = smoke.results.filter((r) => !r.pass);
      if (!failed.length) return "none";
      const head = failed.slice(0, max).map((r) => r.name).join(", ");
      return failed.length > max ? `${head} (+${failed.length - max} more)` : head;
    }
    function formatSmokeDetails(smoke) {
      if (!smoke || !Array.isArray(smoke.results)) {
        return ["Smoke       : not run", "Failures    : not run"].join("\n");
      }
      const lines = [
        `Smoke       : ${formatSmokeSummary(smoke)}`,
        `Failures    : ${smoke.failCount}`
      ];
      const failed = smoke.results.filter((r) => !r.pass);
      if (failed.length) {
        ...
        failed.forEach((r) => {
          lines.push(`  - ${r.name}`);
          if (r.details) lines.push(`    ${String(r.details).replace(/\s+/g, " ").trim()}`);
        });
      } else {
    ...
    function applySmokeDiagnostics(smoke) {
      lastSmokeResults = smoke;
      if (diagSmoke) {
        diagSmoke.textContent = formatSmokeSummary(smoke);
        diagSmoke.style.color = smoke && smoke.failCount > 0 ? "var(--pmt-danger)" : "var(--pmt-success)";
        diagSmoke.title = smoke && smoke.failCount > 0 ? summarizeSmokeFailures(smoke, 10) : "All smoke tests passed";
      }
      if (diagSmokeFailures) {
        diagSmokeFailures.textContent = summarizeSmokeFailures(smoke, 4);
        diagSmokeFailures.style.color = smoke && smoke.failCount > 0 ? "var(--pmt-danger)" : "var(--pmt-text-muted, #8b949e)";
      }
    }
    async function ensureSmokeResults() {
      if (lastSmokeResults && Array.isArray(lastSmokeResults.results)) return lastSmokeResults;
      const { runSmokeTests: runSmokeTests2 } = await Promise.resolve().then(() => (init_smoketests(), smoketests_exports));
      const smoke = runSmokeTests2();
      applySmokeDiagnostics(smoke);
      return smoke;
    }
    if (runSmokeBtn) {
    ...
    if (copyDiagBtn) {
      copyDiagBtn.addEventListener("click", async () => {
        try {
          const smoke = await ensureSmokeResults();
          const auto = getAutomationState();
          const report = [
            ...
            `Host        : ${auto.status} (${auto.confidence.toFixed(2)} confidence)`,
            `History     : ${formatHistory(5)}`,
            `Failing     : ${summarizeSmokeFailures(smoke, 10)}`,
            formatSmokeDetails(smoke),
            `User agent  : ${navigator.userAgent.slice(0, 160)}`,
...
`);
    ...
    function _renderSnapshots() {
      ...
      if (!snaps.length) {
        ...
        outEl.innerHTML = `<div class="pmt5-snap-empty">
          <span class="${NS}-hint">No snapshots yet \u2014 save a restore point before trimming or applying changes.</span>
          <div class="pmt5-snap-toolbar">
            <button class="pmt5-btn-tool pmt5-snap-toolbar-btn" id="${NS}-snap-save-now" ${canSave ? "" : "disabled"}>\uD83D\uDCF7 Save Snapshot${canSave ? "" : " (trim first)"}</button>
            <label class="pmt5-btn-tool pmt5-snap-toolbar-btn pmt5-snap-import-label" title="Import previously exported snapshot JSON">
              \uD83D\uDCC2 Import JSON
      ...
      const exportBtn = `<div class="pmt5-snap-toolbar">
      <span class="pmt5-snap-toolbar-meta">${snaps.length} snapshot${snaps.length === 1 ? "" : "s"} saved for this scope</span>
      <button class="pmt5-btn-tool pmt5-snap-toolbar-btn" id="${NS}-snap-save-now" ${canSave2 ? "" : "disabled"}>\uD83D\uDCF7 Save</button>
      <button class="pmt5-btn-tool pmt5-snap-toolbar-btn" id="${NS}-snap-export-all">\u2B07 Export JSON</button>
      <label class="pmt5-btn-tool pmt5-snap-toolbar-btn pmt5-snap-import-label" title="Import snapshot JSON file">
        \uD83D\uDCC2 Import JSON
        <input type="file" id="${NS}-snap-import-file" accept=".json" style="display:none">
      </label>
      <button class="pmt5-btn-tool lore pmt5-snap-toolbar-btn" id="${NS}-snap-lore-dl" ${canLore ? "" : "disabled"} title="Download current entries as a lore draft .txt file">\uD83D\uDCD6 Lore \u2B07</button>
    </div>`;
    ...
    function buildSettings() {
      ...
      if (apiKeyInput) {
        try {
          apiKeyInput.value = (_i = getStoredKey()) != null ? _i : "";
        } catch {
        }
        apiKeyInput.addEventListener("change", () => {
          ...
          try {
            ...
            applyCapabilityGating();
            if (onboardingBanner) {
              const helpBody = onboardingBanner.querySelector('[data-pmt-help="body"]');
              if (helpBody) helpBody.innerHTML = buildHelpContent(buildCapabilities(cfg));
            }
    ...
    outEl.addEventListener("click", (e) => {
      var _a2;
      const badge = e.target.closest(".pmt5-annot-badge, .pmt5-annot-open");
  ...
  function createMountObserver({ sentinel, fab, styleEl, tryInjectShortcut }) {
    ...
    const observer = new MutationObserver(() => {
      ...
      reinjectTimer = setTimeout(() => {
        ...
        // Keep injected mini-toolbar in sync with host remounts and setting changes
        const cfg2 = (() => { try { return JSON.parse(localStorage.getItem("pmt5_cfg") || "{}"); } catch { return {}; } })();
        const bestWin = findBestMemoryWindow();
        [...document.querySelectorAll(".window")].forEach((win) => {
          if (!cfg2.injectMiniToolbar || !bestWin || win !== bestWin.el) {
            removeMiniToolbar(win);
          }
        });
        if (cfg2.injectMiniToolbar && (bestWin == null ? void 0 : bestWin.el)) {
          injectMiniToolbar(bestWin.el, { onOpen: () => openPanel() });
        }
      }, 250);
    });
...
#pmt5-onboard .pmt5-onboard-body { flex: 1; }
...
.pmt5-bubble-overlay-shell {
  ...
  animation:pmt5-fade-in 0.2s ease;
}
.pmt5-bubble-shell-sized {
  width:min(1040px,96vw);
  height:min(760px,94vh);
}
.pmt5-bubble-slider {
  width:70px;
  accent-color:var(--pmt-accent,#388bfd);
  vertical-align:middle;
  margin-left:4px;
}
.pmt5-bubble-btn-compact {
  font-size:11px;
  padding:3px 9px;
}
.pmt5-bubble-apply[disabled] {
  opacity:0.35;
  cursor:default;
}
.pmt5-bubble-reset {
...
.pmt5-bubble-order-hint {
  color:var(--pmt-accent,#388bfd);
}
.pmt5-bubble-canvas-wrap {
  flex:1;
  overflow:auto;
  position:relative;
  padding:0 12px 12px;
